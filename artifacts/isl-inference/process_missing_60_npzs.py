...
def process_video_file(vid_path: Path) -> np.ndarray:
...
for sign in CANDIDATE_SIGNS:
    ...
    existing_npzs = list(sign_dir.glob('*.npz'))
    
    if len(existing_npzs) == 0:
        vids = video_map.get(sign, [])[:3]
        print(f"Extracting missing sign: {sign} from {len(vids)} videos...")
        for p_idx, (stype, vpath) in enumerate(vids):
            ...
            split = 'train' if p_idx == 0 else ('val' if p_idx == 1 else 'test')
            try:
                seq_36 = process_video_file(vpath)
                meta = {
...
# Generate global dataset_manifest.json
manifest = []
for sign in CANDIDATE_SIGNS:
    sign_dir = out_dir / sign
    valid_npzs = sorted(list(sign_dir.glob('*.npz')))
...
print(f"\n==================================================")
print(f"SUCCESSFULLY PROCESSED ALL 60 CLASSES!")
print(f"Total processed clips: {len(manifest)} across {len(unique_classes)} unique classes.")
